...
(test 'cons-4 (cons 'a 3) '(a . 3))
(test 'cons-5 (cons '(a b) 'c) '((a b) . c))

(test-error 'car-1 (car 5))
(test-error 'car-2 (car '()))
(test 'car-3 (car '(a b c)) 'a)
(test 'car-4 (car '((a) b c d)) '(a))
(test 'car-5 (car '(1 . 2)) 1)

(test-error 'cdr-1 (cdr 5))
(test-error 'cdr-2 (cdr '()))
(test 'cdr-3 (cdr '(a)) '())
(test 'cdr-4 (cdr '(a b c)) '(b c))
(test 'cdr-5 (cdr '(1 . 2)) 2)

(test-error 'set-car!-1 (set-car! 0 0))
(test-error 'set-car!-2 (set-car! '() 0))
(test-error 'set-car!-3 (set-car! '(0) 1))
(test 'set-car!-4
  (let ((a (list 'b 'c)))
    (set-car! a '())
	a)
...
(test-error 'set-cdr!-1 (set-cdr! 0 0))
(test-error 'set-cdr!-2 (set-cdr! '() 0))
(test-error 'set-cdr!-3 (set-cdr! '(0) 1))
(test 'set-cdr!-4
  (let ((a (list 'a)))
    (set-cdr! a 'b)
	a)
...
(test 'null?-1 (null? 0) #f)
(test 'null?-2 (null? '()) #t)
(test 'null?-3 (null? '(a . 2)) #f)

(test 'list?-1 (list? 0) #f)
...
(test-error 'reverse-1 (reverse 0))
...
(test 'assq-1 (assq 'a e) '(a 1))
(test 'assq-2 (assq 'b e) '(b 2))
